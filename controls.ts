...
// DOM helper
    ...
    export class CImageLoader {

        items: HTMLElement[] = [];

        constructor(aBaseUrl: string, aImgList: string[]) {
            var i: number;
            for (i = 0 ; i < aImgList.length ; i += 1) {
                var imgEl = document.createElement('img');
                imgEl.setAttribute('src', aBaseUrl + aImgList[i]);
                this.items.push(imgEl);
            }
        }
    }

    export class CMetaDataDisplayUtil {

        _element: HTMLElement;
        _parentEl: HTMLElement;
        _dimmingEl: HTMLElement;
        _metaChannelEl: HTMLElement;
        _metaTitleEl: HTMLElement;
        _metaBottomEl: HTMLElement;
        _metaDurationEl: HTMLElement;
        _metaIconEl: HTMLElement;
        _metaDescriptionEl: HTMLElement;
        _metaDescriptionText: HTMLElement;
        _metaEpisodeEl: HTMLElement;
        _isShown: boolean;
        _imageLoader: CImageLoader;
        _metaDataItem: any;

        constructor(aParentEl: HTMLElement, aImageBaseUrl: string, aDimming: boolean) {
            ...
            this._metaBottomEl.appendChild(this._metaDurationEl);
            this._metaDescriptionEl.appendChild(this._metaDescriptionText);
            this._element.appendChild(this._metaEpisodeEl);
            ...
            var imgs = [
                ...
                'icon_reminder.png',        // 0
                'icon_rec.png',             // 1
                'icon_rec_series.png',      // 2
                // Status
                'icon_kept.png',            // 3
                'icon_watched.png',         // 4
                'icon_watched_part.png',    // 5
                'icon_deleted_soon.png',    // 6
                'icon_failed.png',          // 7
                // Attribute
                'icon_restart.png',         // 8
                'icon_ondemand.png',        // 9
                ...
                'icon_SD_HD.png',           // 12
                'icon_3d.png',              // 13
                'icon_showcase.png',        // 14
                //'icon_5.1audio.png',        // 15
                'icon_dolby.png',
                // Accessability
                'icon_sub.png',             // 16
                'icon_guidance.png',        // 17
                'icon_ad.png',              // 18
                'icon_signed.png',          // 19
            ];
        ...
        destroy() {
            var i: number;
            var len: number = this._imageLoader.items.length
            for (i = 0; i < len; i += 1) {
                this._imageLoader.items[i] = null;
            }
        ...
        setMetaDataProvider(aMetaDataProvider: any) {
            var i: number;
            var len: number;
            var imgElArray: HTMLElement[];
        ...
        showExt() {
            if (this._metaDataItem) {
                var channelName: string = this._metaDataItem.getChannelName();
                var title: string = this._metaDataItem.getTitle();
                ...
                //        });
                //    }
                this._metaEpisodeEl.innerText = seasonEpisodeInfo;
                this._element.appendChild(this._metaEpisodeEl);
                this._element.style.display = 'block';
                this._isShown = true;
                return true;
        ...
        hide() {
            ...
            this._metaDurationEl.innerText = '';
            this._metaDescriptionText.innerText = '';
            this._metaEpisodeEl.innerText = '';
            this._metaIconEl.innerHTML = '';
            this._element.style.display = 'none';
            this._isShown = false;
        }
    ...
    export class CPasswordControl extends CControl {

        _passwordNodes: HTMLElement[] = [];
        _passwordNos: number[] = [];
        _count: number;
        _element: HTMLElement;

        constructor(aElement: HTMLElement, aCount: number) {
            super(aElement);
            ...
            for (i = 0 ; i < this._count; i += 1) {
                ...
                this._passwordNodes.push(_passwordNode);
                this._passwordNos.push(-1);
                this._element.appendChild(_passwordNode);
            }
            this._keyMapBuilder = KBuilderLeftRight;
        }

        setClassName(aString: string) {
        ...
        clear() {
            ...
            for (i = 0 ; i < this._count ; i += 1) {
                ...
                this._passwordNodes[i].innerText = '';
                this._passwordNos[i] = -1;
            }
        ...
        doKeyNumber(aNum: number): boolean {
            var i: number;
            ...
            for (i = 0 ; i < this._count; i += 1) {
                if (this._passwordNodes[i].classList.contains(KClassActiveFocusedLeaf)) {
                    ...
                    clearTimeout(this['_hideTimer' + i]);
                    /* TODO: find right way
                    this['_hideTimer' + i] = setTimeout(((aIndex) => {
                        return function () => {
                            this._passwordNodes[aIndex].innerText = '*';
                        };
                    }(i)), 200);
                    */
                    break;
    ...
    export class CButton extends CControl {
        ...
        setButtonText(aText: string) {
            this._element.innerText = aText;
        }

        setOkHandler(aHandler) {
